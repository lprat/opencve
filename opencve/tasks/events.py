...
def get_exploited_cve_from_rss():
    ...
    if not fstate:
        ...
        if "last_time" in fstate:
            ...
            if int((datetime.now() - dlast).total_seconds()) < int(
                cel.app.config["UPDATE_RSS"]
            ):
                logger.info(
                    "Dont check update RSS (wait {}seconds between 2 checks).".format(
                        cel.app.config["UPDATE_RSS"]
                    )
                )
                return cve_exploited
...
def update_exploited_cve_from_rss(cve_exploited, task):
    ...
    for cve_id in cve_exploited:
        ...
        event = CveUtil.create_event(
            cve_obj, cve_obj.json, "exploited", {"old": False, "new": True}
        )
        if event:
            events.append(event)
...
def check_for_update(cve_json, task, exploit_db=[]):
    ...
    # Existing CVE has changed
        ...
        if cve_obj.exploit != old_exploit:
            event = CveUtil.create_event(
                cve_obj, cve_json, "exploit", {"old": False, "new": True}
            )
            if event:
                events.append(event)
